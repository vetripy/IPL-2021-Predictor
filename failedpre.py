...
def score(matchid):
    ...
    if cond!=[1,2]:
        ...
        balls = new.groupby(['runs_off_bat','innings','bowler']).size().reset_index(name='index')
        balls=balls[(balls.runs_off_bat==0)]
        dot_balls = new.groupby(['venue', 'innings', 'batting_team','bowler'])[['player_dismissed','wides','noballs']].count()
